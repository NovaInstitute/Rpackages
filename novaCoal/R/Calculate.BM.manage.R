...
# % Nova BASA Users Retention Actual
# Nova BASA Users Carried forward Actual	
# FACTOR VERS per Household existing Actual	
# FACTOR VERS per Household New Actual	
# % New user start date Impact Actual	
# Low estimate Actual

# Hierdie ding moet 'n gesmelte df as uit voer hÃª
#################################################################################

calculate.BM.manage <- function(x, verbose=FALSE, debug = FALSE, COEF = 2.3515, naampos = NULL
                                   ){
  ...
  haq[is.na(haq$response.id), "response.id"] <- haq[is.na(haq$response.id), "ï..submission.id"]
