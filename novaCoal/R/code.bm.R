...
code.bm <- function(haq=haq,
                    coal.use = "coal.use",
                    contact = "contact",
                    project.startyear = 2007,
                    verbose = FALSE){
# Recode multi.firemaker..coal.ignition
names(haq) = gsub("_0", "", names(haq))
names(haq) = gsub(".1", "", names(haq))
...
### A Bottom Up user cannot report no savings because of BM since she does not use BM. Sometimes the fieldworker marks "No" instead of "Nobody in this house uses BM"
