#' Coal Merchant Survey (Main function posessing various internal functions)
#'
#' Script to subset coal merchant survey for sepcific towns and create merchant an price latex tables.
#'
#' @param csv If null the function chooses the csv option whereby coal merchant survey data is read
...
#' @param fn The filename of coal merchant survey data to be used if read.csv is null
#' @param manual.edit Logical that initializes a manual edit when TRUE
#' @param manual.dest manual destination to be added to the town column
#' @param selectvar The selection variables to be used as a factor
#' @param fn.s The file name and directory to which the coal merchant data should be saved
#' as character vector
#' @param tabdirr The directory to which any tables should be saved as character vector
#' @param loc The locations under consideration as a factor
#' @param format The coal buying format information as a factor
#' @param debug Logical that assigns data to gobal environment for debugging process when TRUE
#' @param verkort Logical that shortens data by matching town data to selection variable data
#' @param subplace If TRUE the function melts and reconstructs the given data frame with
...
#' Merchant Mean (internal function)
#'
#' Function is used to work out the mean weight per forat type for every merchant
#'
#' @param x Numeric vecotr referring to the length of coal merchant data i.e the amount of merchants
#' @param data Data frame containing coal merchant information
#' @param format.idx The coal format under consideration
#' @param nr.measured The variable containing the numer of the format under consideration as character vector
#' @export
...
#' Bag Price (internal function)
#'
#' Function to calculate bag prices
#'
...
#' @param data Data frame containing coal merchant data
#' @param fromat.price The price of the format under consideration as character vector
#' @param mean weight The mean weight of the format under consideration as character vector
#' @export
...
if (numvar > 3){col.add =  paste(rep("p{1.2cm}",numvar-3),sep="",collapse="")} else {col.add = ""}
...
p.col.add = paste(rep("p{1.2cm}",p.numvar-2),sep="",collapse="")
message(p.col.add)
...
#' Summarise Coal Merchant (Internal function)
#'
#' This function creates a summary data frame containing the coal format, amount of each
#' format and min, mean and max weights
#'
#' @param coalm Data frame containing coal merchant survey data
#' @export
