##########################################################
# Print table to be used for multi option questions      #
##########################################################
cap.tN = function(x, cap = m.cap,
                  lab = m.lab,
  ...
  if(dim(x)[2] != 0){
    capture.output(tableNominal(x, cap = cap, lab = lab,
                                cumsum = cumsum, nams = names(x), group = group),
                   file = paste(dir, lab, ".tex", sep = ""))
  }
